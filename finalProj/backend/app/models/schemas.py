...
CallerContext = Literal["known_contact", "unknown_contact", "not_provided"]
TransactionType = Literal["fund_transfer", "otp_or_pin_request", "account_update", "other", "not_provided"]
AdvisoryRecommendation = Literal["continue_with_caution", "pause_and_verify", "block_and_report"]


class SimulationContext(BaseModel):
    """
    Validated call and transaction context model for demo advisory evaluation.
    Does NOT accept free-form personal names, phone numbers, or account/OTP credentials.
    """
    caller_context: CallerContext = Field(
        "not_provided",
        description="Caller relationship status (known_contact, unknown_contact, not_provided)"
    )
    transaction_type: TransactionType = Field(
        "not_provided",
        description="Type of transaction or action requested during call"
    )
    transaction_amount: Optional[float] = Field(
        None,
        gt=0,
        description="Optional transaction amount for demo display (must be positive)"
    )
...
class AdvisoryResult(BaseModel):
    """
    Explainable advisory output generated by the contextual policy layer.
    Included as an optional additive field in streamed RiskUpdate WebSocket messages.
    """
    recommendation: AdvisoryRecommendation = Field(
        ...,
        description="Recommended action: continue_with_caution, pause_and_verify, or block_and_report"
    )
    reason_codes: List[str] = Field(
        default_factory=list,
        description="Stable machine-readable reason codes explaining the recommendation"
    )
    user_message: str = Field(
        ...,
        description="Short, safe user-facing explanatory sentence"
    )
    requires_user_confirmation: bool = Field(
        True,
        description="Whether explicit user confirmation is recommended"
    )
...
class SimulationRequest(BaseModel):
    ...
    scenario: Literal["default", "clean", "suspicious", "gradual_escalation", "human_voice", "clone_attack"] = Field(
        "gradual_escalation",
    ...
    context: Optional[SimulationContext] = Field(
        None,
        description="Optional call and transaction context for demo advisory policy evaluation."
    )
