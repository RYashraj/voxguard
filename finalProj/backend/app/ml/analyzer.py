...
async def analyze_chunk_dispatch(
    ...
    - 'real': Runs real ML model inference off the main event loop thread via asyncio.to_thread.
    - 'stub' (default): Runs simulated stub scenario for development/testing.
    ...
    """
    raw_mode = os.getenv("VOXGUARD_ML_MODE", "stub").lower().strip()
