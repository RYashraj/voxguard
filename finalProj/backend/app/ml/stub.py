...
def analyze_chunk_stub(
    ...
    if scenario == "clean":
        ...
        # Clean confirmed human voice (mic live input - stays solidly green)
        base_score = 0.05 + random.uniform(-0.02, 0.04)
        chunk_score = round(max(0.01, min(0.18, base_score)), 4)
        confidence = round(random.uniform(0.94, 0.99), 4)
        flags = []

    elif scenario == "clone_attack":
        # ElevenLabs voice clone detected — immediately high risk from chunk 1
        # Simulates spectral artifacts + prosody discontinuities of neural TTS cloning
        base_score = 0.82 + random.uniform(-0.04, 0.10)
        chunk_score = round(max(0.78, min(0.97, base_score)), 4)
