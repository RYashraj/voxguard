...
- **`VOXGUARD_ML_MODE=stub`** *(Demo default)*: Runs simulated score scenario streams for local UI testing without requiring model weights.
- **`VOXGUARD_ML_MODE=real`**: Runs real synchronous Spectra-AASIST3 model inference safely off the main event loop thread via `asyncio.to_thread`.
