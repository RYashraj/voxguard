...
- **Hugging Face Model Cache**: Optional real-mode weights are read from `HF_HOME`; if unset, Hugging Face uses its standard per-user cache. Tonight's demo uses stub mode by default.
...
|---|---|---|
| **Model Unavailable flag** | Real mode selected without cached weights | Use `$env:VOXGUARD_ML_MODE = "stub"` for the deterministic demo, or set `HF_HOME` to a valid Hugging Face cache before selecting real mode. |
